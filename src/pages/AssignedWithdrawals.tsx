...
export const AssignedWithdrawals = () => {
  ...
  const handleApprove = async (id: string) => {
    try {
      const res = await updateWithdrawPaidStatus({
        diamondWIthdrawId: id,
      }).unwrap();
      toast.success(
        res.message || "Diamond withdraw updated status successfully",
      );
    } catch (err) {
      console.log(err);
    }
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="mb-6">
        <h2 className="text-2xl font-black text-slate-900 dark:text-slate-100">
          My Assigned Withdrawals
        </h2>
        <p className="text-slate-100 text-sm mt-1">
          Mark them as paid once the bank transfer is verified.
        </p>
      </div>
      <DataTable
        data={withdrawals}
